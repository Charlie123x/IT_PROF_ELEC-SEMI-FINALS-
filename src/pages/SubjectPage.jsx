      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-gray-400 mt-1">Manage and view all subjects and courses</p>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-yellow-600/20 to-yellow-800/20 border-b border-yellow-600 border-opacity-30">
                  ...
                  <th className="px-6 py-4 text-left text-yellow-400 font-semibold">Subject Code</th>
                  <th className="px-6 py-4 text-left text-yellow-400 font-semibold">Subject Name</th>
                  <th className="px-6 py-4 text-left text-yellow-400 font-semibold">Instructor</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  (subjects.length ? subjects : fallbackData).map((subject) => (
                    <tr key={subject.id} className="border-b border-gray-800 hover:bg-yellow-600/10 transition-colors">
                      <td className="px-6 py-4 text-gray-300">{subject.id}</td>
                      <td className="px-6 py-4 text-yellow-400 font-mono font-semibold">{read(subject, dbKeys.codeKey)}</td>
                      <td className="px-6 py-4 text-white font-medium">{read(subject, dbKeys.nameKey)}</td>
                      <td className="px-6 py-4 text-gray-300">{read(subject, dbKeys.instructorKey)}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => handleOpenModal(subject)} className="p-2 bg-yellow-600 hover:bg-yellow-700 text-black rounded-lg transition-all hover:scale-110">
                            <Edit size={16} />
                          </button>
                          <button onClick={() => handleDelete(subject.id)} className="p-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-all hover:scale-110">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-400">
            Total Subjects: <span className="text-yellow-400 font-semibold">{subjects.length || fallbackData.length}</span>
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 backdrop-blur-sm p-4">
          <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl border border-yellow-600 border-opacity-30 shadow-2xl shadow-yellow-600/50 w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-yellow-400 font-semibold mb-2">Subject Code</label>
              ...
              <div>
                <label className="block text-yellow-400 font-semibold mb-2">Subject Name</label>
              ...
              <div>
                <label className="block text-yellow-400 font-semibold mb-2">Instructor</label>
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={handleCloseModal} className="flex-1 px-6 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl font-semibold transition-all">
                  Cancel
                </button>
                <button type="submit" disabled={loading} className="flex-1 px-6 py-3 bg-gradient-to-r from-yellow-600 to-yellow-800 hover:from-yellow-700 hover:to-yellow-900 text-black rounded-xl font-semibold shadow-lg shadow-yellow-600/50 transition-all hover:scale-105 flex items-center justify-center gap-2">
                  <Save size={20} />
